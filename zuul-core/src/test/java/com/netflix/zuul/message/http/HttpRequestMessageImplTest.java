...
class HttpRequestMessageImplTest {
    ...
    void testOriginalRequestInfo() {
        ...
        assertEquals(request.getQueryParams().getFirst("flag"), originalRequest.getQueryParams().getFirst("flag"));
    ...
    void testPathAndQuery() {
        HttpQueryParams queryParams = new HttpQueryParams();
        queryParams.add("flag", "5");
        request = new HttpRequestMessageImpl(new SessionContext(), "HTTP/1.1", "POST", "/some/where", queryParams, new Headers(), "192.168.0.2", "https", 7002, "localhost");
    ...
    void testPathAndQuery_immutable() {
        HttpQueryParams queryParams = new HttpQueryParams();
        queryParams.add("flag", "5");
        request = new HttpRequestMessageImpl(new SessionContext(), "HTTP/1.1", "POST", "/some/where", queryParams, new Headers(), "192.168.0.2", "https", 7002, "localhost", new SocketAddress() {
        }, true);
        // Check it's the same value 2nd time.
        assertEquals("/some/where?flag=5", request.getPathAndQuery());
        assertEquals("/some/where?flag=5", request.getPathAndQuery());
    ...
    void getOriginalHost_fallsBackOnUnbracketedIpv6Address_WithNonStrictValidation() {
        ...
        Headers headers = new Headers();
        headers.add("Host", "ba::dd");
        request = new HttpRequestMessageImpl(new SessionContext(), "HTTP/1.1", "POST", "/some/where", queryParams, headers, "192.168.0.2", "https", 7002, "server");
