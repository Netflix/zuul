    ...
    private void logPassport(Channel channel) {
        ...
        if (LOG.isInfoEnabled()) {
            ...
            if (passport.calculateTimeBetween(inReqToOutResp) > WARN_REQ_PROCESSING_TIME_NS.get()) {
                LOG.info("Request processing took longer than threshold! toplevelid = {}, {}",
                        topLevelRequestId, ChannelUtils.channelInfoForLogging(channel));
            }
            ...
            if (passport.calculateTimeBetween(inRespToOutResp)
                ...
                LOG.info("Response processing took longer than threshold! toplevelid = {}, {}",
                        topLevelRequestId, ChannelUtils.channelInfoForLogging(channel));
            }
        }
    }
