        ...
        public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
            try {
                ...
                if (msg instanceof HttpRequest) {
                    ...
                    promise.addListener(new PassportStateListener(passport, PassportState.OUT_REQ_HEADERS_SENT, PassportState.OUT_REQ_HEADERS_ERROR_SENDING));
                }
                if (msg instanceof LastHttpContent) {
                    passport.add(PassportState.OUT_REQ_LAST_CONTENT_SENDING);
                    promise.addListener(new PassportStateListener(passport, PassportState.OUT_REQ_LAST_CONTENT_SENT, PassportState.OUT_REQ_LAST_CONTENT_ERROR_SENDING));
                } else if (msg instanceof HttpContent) {
                    passport.add(PassportState.OUT_REQ_CONTENT_SENDING);
                    promise.addListener(new PassportStateListener(passport, PassportState.OUT_REQ_CONTENT_SENT, PassportState.OUT_REQ_CONTENT_ERROR_SENDING));
                }
            } finally {
