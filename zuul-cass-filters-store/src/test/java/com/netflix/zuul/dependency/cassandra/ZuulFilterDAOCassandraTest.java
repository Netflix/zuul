...
public class ZuulFilterDAOCassandraTest {
    @Mock
    ZuulFilterDAOCassandra.CassandraGateway gateway;
    @Mock
    ...
    public void testGetAllFiltersReturnsEmptyListInsteadOfNullWhenNoFilters() {
        ...
        Mockito.when(gateway.select(Matchers.anyString())).thenReturn(response);
        Mockito.when(response.isEmpty()).thenReturn(true);
    ...
    public void testGetFilterIdsRawIndex() {
        ...
        Iterator<Row<String, String>> iterator = (Iterator<Row<String, String>>) Mockito.mock(Iterator.class);
        Mockito.when(response.iterator()).thenReturn(iterator);
        Mockito.when(iterator.hasNext()).thenReturn(true, false); // 1 row
        Mockito.when(iterator.next()).thenReturn(row0, (Row) null);

        Mockito.when(gateway.select(Matchers.anyString())).thenReturn(response);
        Mockito.when(response.isEmpty()).thenReturn(false);
        Mockito.when(response.size()).thenReturn(1);

            /* exercise the method we're testing */
        String list = dao.getFilterIdsRaw("index");

            /* validate responses */
        Assert.assertEquals(fids, list);
    ...
    public void testGetFilterIdsIndex() {
        ...
        Assert.assertEquals(list.get(0), "filter1");
        Assert.assertEquals(list.get(1), "filter2");
        Assert.assertEquals(list.get(2), "filter3");
    ...
    public void testGetAllEndpointsReturnsResults() {
        ...
        List<String> list = dao.getAllFilterIDs();
            ...
            /* validate responses */
        Assert.assertEquals("filter1", list.get(0));
        Assert.assertEquals("filter2", list.get(1));
    }
    ...
    public void testGetScriptsForFilterReturnsResults() {
        ...
        Mockito.doReturn("name:type_1|name:type_2").when(dao).getFilterIdsRaw(Matchers.anyString());
        ...
        Iterator<Row<String, String>> iterator = (Iterator<Row<String, String>>) Mockito.mock(Iterator.class);
        Mockito.when(response.iterator()).thenReturn(iterator);
        Mockito.when(iterator.hasNext()).thenReturn(true, true, false); // 2 rows
        Mockito.when(iterator.next()).thenReturn(row0, row1, null);


        Mockito.when(response.isEmpty()).thenReturn(false);
        Mockito.when(response.size()).thenReturn(2);

        Mockito.when(gateway.getByFilterIds(Matchers.anyList())).thenReturn(response);

            /* exercise the method we're testing */
        List<FilterInfo> list = dao.getZuulFiltersForFilterId(filter);

            /* validate responses */
        Assert.assertEquals(filter, list.get(0).getFilterID());
        Assert.assertEquals(1, list.get(0).getRevision());
        Assert.assertEquals(true, list.get(0).isActive());
        Assert.assertEquals(now.getTime(), list.get(0).getCreationDate());
        // assert using scriptsForExecution
        Assert.assertEquals(filter, list.get(1).getFilterID());
        Assert.assertEquals(2, list.get(1).getRevision());
        Assert.assertEquals(false, list.get(1).isActive());

        // assert using scriptsForExecution
        // now assert using filenames to lookup
        Assert.assertEquals("script body 2a", list.get(1).getFilterCode());
    ...
    public void testGetScriptForEndpointAndRevision() {
        ...
        Iterator<Row<String, String>> iterator = (Iterator<Row<String, String>>) Mockito.mock(Iterator.class);
        Mockito.when(response.iterator()).thenReturn(iterator);
        Mockito.when(iterator.hasNext()).thenReturn(true, false); // 2 rows
        Mockito.when(iterator.next()).thenReturn(row0, (Row) null);


        Mockito.when(response.getRowByIndex(0)).thenReturn(row0);
        ...
        Mockito.doReturn("name:type_3").when(dao).getFilterIdsRaw(Matchers.anyString());
        ...
        FilterInfo filterInfo = dao.getFilterInfoForFilter(filter, 3);
            ...
            /* validate responses */
        Assert.assertEquals(filter, filterInfo.getFilterID());
        Assert.assertEquals(3, filterInfo.getRevision());
        Assert.assertEquals(true, filterInfo.isActive());
        Assert.assertEquals(now.getTime(), filterInfo.getCreationDate());
        Assert.assertEquals("script body 1", filterInfo.getFilterCode());
    }
    ...
    public void testGetScriptForLatestEndpoint() {
        ...
        Mockito.doReturn("name:type_4").when(dao).getFilterIdsRaw(Matchers.anyString());
        ...
        FilterInfo filterInfo = dao.getLatestFilterInfoForFilter(filter);
            ...
            /* validate responses */
        Assert.assertEquals(filter, filterInfo.getFilterID());
        Assert.assertEquals(4, filterInfo.getRevision());
        Assert.assertEquals(false, filterInfo.isActive());
        Assert.assertEquals(now.getTime(), filterInfo.getCreationDate());
        Assert.assertEquals("script body 1", filterInfo.getFilterCode());
    }
    ...
    public void testAddScriptForNewEndpointUsingArray() {
        ...
        ZuulFilterDAOCassandra.CassandraGateway testGateway = new ZuulFilterDAOCassandra.CassandraGateway() {
            @Override
        ...
        dao = Mockito.spy(dao);
        Mockito.doReturn("name:type").when(dao).getFilterIdsRaw(Matchers.anyString());
        Mockito.when(gateway.getByFilterIds(Matchers.anyList())).thenReturn(response);
        ...
        Assert.assertEquals(filter + "_1", upsertedRowKey.toString());
        Assert.assertEquals(filter, upsertedAttributes.get("filter_id"));
        Assert.assertEquals(1L, upsertedAttributes.get("revision"));
        Assert.assertEquals(false, upsertedAttributes.get("active"));
        Assert.assertTrue(upsertedAttributes.get("creation_date") instanceof Date);
        Assert.assertTrue(Arrays.equals("code".getBytes(), (byte[]) upsertedAttributes.get("filter_code")));
    ...
    public void testAddScriptForExistingEndpoint() {
        ...
        testGateway = Mockito.spy(testGateway);
            /* exercise the method we're testing */
        ZuulFilterDAO dao = new ZuulFilterDAOCassandra(testGateway);

        dao = Mockito.spy(dao);
        Mockito.doReturn("name:type").when(dao).getFilterIdsRaw(Matchers.anyString());
        Mockito.doReturn(response).when(testGateway).getByFilterIds(Matchers.anyList());
        ...
        Assert.assertEquals("null:" + filter + "_2", upsertedRowKey.toString());
        Assert.assertEquals(2L, upsertedAttributes.get("revision"));
    }
    ...
    public void testSetScriptActive() {
        ...
        Mockito.when(response.getRowByIndex(1)).thenReturn(row1);
        ...
        Iterator<Row<String, String>> iterator = (Iterator<Row<String, String>>) Mockito.mock(Iterator.class);
        Mockito.when(response.iterator()).thenReturn(iterator);
        Mockito.when(iterator.hasNext()).thenReturn(true, true, false); // 2 row
        Mockito.when(iterator.next()).thenReturn(row0, row1);

            /* define a 2nd response that is returned as the FilterInfo from the method */
        Rows<String, String> response2 = Mockito.mock(Rows.class);
        ...
        Mockito.when(response2.getRowByIndex(0)).thenReturn(response2row0);

        Mockito.when(response2.isEmpty()).thenReturn(false);
        Mockito.when(response2.size()).thenReturn(1);


        Rows<String, String> response3 = Mockito.mock(Rows.class);
        ...
        Mockito.when(response3.getRowByIndex(0)).thenReturn(response3row0);

        Mockito.when(response3.isEmpty()).thenReturn(false);
        Mockito.when(response3.size()).thenReturn(1);

        Iterator<Row<String, String>> iterator1 = (Iterator<Row<String, String>>) Mockito.mock(Iterator.class);
        Mockito.when(response3.iterator()).thenReturn(iterator1);
        Mockito.when(iterator1.hasNext()).thenReturn(true, false); // 1 row
        ...
        dao = Mockito.spy(dao);
        Mockito.doReturn("name:type").when(dao).getFilterIdsRaw(Matchers.anyString());
        Mockito.when(gateway.getByFilterIds(Matchers.anyList())).thenReturn(response3, response, response2);
        ...
        inOrder.verify(gateway, Mockito.times(1)).upsert(filter + "_4", attributesForActivation);
    ...
    public void testSetScriptActiveHandlesPartialFailure() {
        ...
        attributesForActivation.put("active", true);
        attributesForActivation.put("canary", false);
        Mockito.doThrow(new RuntimeException()).when(gateway).upsert(filter + "_4", attributesForActivation);
        ...
        Map<String, Object> attributesForDeactivation = new HashMap<String, Object>();
        attributesForDeactivation.put("active", false);
        Mockito.verify(gateway, Mockito.times(0)).upsert(filter + "_3", attributesForDeactivation);
    }
    ...
    public void testSetScriptActiveDoesNotDeactivateWhatWasJustActivated() {
        ...
        attributesForActivation.put("active", true);
        attributesForActivation.put("canary", false);
        inOrder.verify(gateway, Mockito.times(1)).upsert(filter + "_3", attributesForActivation);

        // ensure we do NOT deactivate it since this is the same revision
        Map<String, Object> attributesForDeactivation = new HashMap<String, Object>();
        attributesForDeactivation.put("active", false);
        inOrder.verify(gateway, Mockito.times(0)).upsert(filter + "_3", attributesForDeactivation);
    }
    ...
    private static Column<String> mockColumn(ColumnList<String> columnList, String columnName, Object value) {
        ...
        Column<String> column = Mockito.mock(Column.class);
        if (value instanceof String) {
            Mockito.when(column.getStringValue()).thenReturn((String) value);
        } else if (value instanceof byte[]) {
            Mockito.when(column.getByteArrayValue()).thenReturn((byte[]) value);
        } else if (value instanceof Date) {
            Mockito.when(column.getDateValue()).thenReturn((Date) value);
        } else if (value instanceof Integer) {
            Mockito.when(column.getIntegerValue()).thenReturn((Integer) value);
        } else if (value instanceof Long) {
            Mockito.when(column.getLongValue()).thenReturn((Long) value);
        } else if (value instanceof Boolean) {
            Mockito.when(column.getBooleanValue()).thenReturn((Boolean) value);
        } else {
            throw new RuntimeException("unsupported type, add another else above");
        }
            /* assign column to columnList for given columnName */
        Mockito.when(columnList.getColumnByName(columnName)).thenReturn(column);
        return column;
    }
    ...
    private static Row<String, String> mockRow() {
        Row<String, String> row0 = Mockito.mock(Row.class);
        return row0;
    }
    ...
    private static void mockGetScriptForEndpoint(Rows<String, String> response, String filter) {
        ...
        Mockito.when(response.isEmpty()).thenReturn(true, false); // true on the first request, false thereafter
        Mockito.when(response.size()).thenReturn(1);
    }
}
